...
Recall the table assignment problem: our goal is to seat guests at each table in a manner that maximizes their similarity..
...
Maximizes their similarity $\Rightarrow$ the shortest distance among points in the same cluster.
...
Find the sets $\{C_{1},C_{2},\cdots C_{K}\}$ such that the within-cluster variation $$W\left(C_{k}\right)=\frac{1}{|C_{k}|}\sum_{i,i'\in C_{k}} \sum_{j=1}^p \left(\mathbf{x}_{i,j}-\mathbf{x}_{i',j}\right)^2  $$ is minimized. Here $\left(\mathbf{x}_{i,j}-\mathbf{x}_{i',j}\right)^2$ is squared Euclidean distance. $$\tag{12.17} \min_{C_{1},C_{2},\cdots C_{K}}\sum_{k=1}^{K}W\left(C_{k}\right)$$
...
-   Objective: minimize $2\sum_{i,i'\in C_{k}}\sum_{j=1}^{p}\left(\mathbf{x}_{i,j}-\bar{\mathbf{x}}_{k,j}\right)^{2}$

-    The step "**Reassign cluster**" is to reallocate $\mathbf{x}_{i,j}$ to improve (reduce) $W\left(C_{k}\right)$

-   The step "**Update centroid**:" is to reallocate "centroid" (cluster means) to minimize the deviations within each group. ( $\frac{d^2W\left(C_{k}\right)}{d\bar{\mathbf{x}}^2_{k,j}}<0$ )

# Discussions

## Which is the 'best' clustering?

Using "total within-cluster sum of squares" as the measurement.
...
-   Sensitive to outliers - consider the soft K-means clustering (Fuzzy clustering [@bezdek2013pattern])
-   Caution when interpreting clustering results (there's no straightforward method to validate the outcomes)
...
#| code-line-numbers: "1-4|6-12|14-16"
#| output-location: slide
#| fig-cap: data distribution
set.seed(124) # Ensures reproducibility
# Simulate 3 groups of 100 points for given means and standard deviations.
ssize <- 100
means <- c( -1, 1, 4)
m     <- c( 1, 9, 5)

# Generate and append group points to a dataframe in a loop.
pts <- data.frame()
for (i in seq_along(means)) {
  x_vals <- rnorm(ssize, mean=means[i], sd=1)
  y_vals <- m[i]* x_vals + rnorm(ssize, mean=0, sd=4) # y = mx + N(0,1)
  group_df <- data.frame(V1=x_vals, V2=y_vals, Ptype=i)
  pts <- rbind(pts, group_df)}

# Plotting with ggplot2, using Ptype as the color factor.
ggplot(pts, aes(x=V1, y=V2, color=factor(Ptype))) + geom_point() +
...
createClusterPlot <- function(pts_df, centers_df, iteration, withinss) {
  p <- ggplot() + 
    ...
    labs(title = paste("Iteration", iteration, "; Total withinss=",  round(sum(withinss), 2))) +  # Add iteration title
  ...
  for (j in 1:nrow(centers_df)) {
    p <- p + annotate("text", x = centers_df[j, 1], y = centers_df[j, 2], label = round(withinss[j], 2), hjust = 1.5, vjust = -0.5)
  }
  return(p)
}
...
for (i in 1:4) {
  ...
  pts_df <- pts %>% mutate(cluster = as.factor(km.out$cluster))
  
  # Prepare the data frame for the cluster centers
  centers_df <- as.data.frame(km.out$centers) %>% mutate(cluster = 1:K)
  
  # Generate the plot using the separate function
  p <- createClusterPlot(pts_df, centers_df, i, km.out$withinss)
